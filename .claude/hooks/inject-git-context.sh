...
# If modules are nested deeper (e.g. services/order-service), increase -maxdepth to 3.
...
# Change to -maxdepth 3 for nested layouts (e.g. services/order-service/pom.xml).
MODULES=$(find "$REPO_ROOT" -maxdepth 2 -name pom.xml 2>/dev/null \
  | grep -v "^$REPO_ROOT/pom.xml$" \
  | sed "s|$REPO_ROOT/||;s|/pom.xml||" \
  | sort | tr '\n' ' ')

# Extract issue number from branch name if present (e.g. feat/order-service-142-retry)
