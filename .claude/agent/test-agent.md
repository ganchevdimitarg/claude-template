...
description: >
  ...
  improve coverage, add Kafka/Avro integration tests, add WireMock stubs for HTTP clients,
...
| Kafka Avro deserialization | Integration — assert every field, use `MockSchemaRegistryClient` |
...
- Excluded: generated Avro classes, `@SpringBootApplication`, `@Configuration` classes

**AbstractIntegrationTest — never redeclare containers:**
All four containers (PostgreSQL, MongoDB, Redis, Kafka) are started once per suite in
`AbstractIntegrationTest` from `common-test`. Extend it — do not redeclare any container.
...
## Invariants
- Never use H2 or EmbeddedMongo — Testcontainers only.
...
4. Any WireMock stubs or Avro schema assumptions made
